...
__device__
void projectPoint(double src[2], double pt[2], double x, double *ret)
{
  if(src[0] == pt[0])
  {
    *ret = pt[0];
...
{
  projectPoint(src, pt1, x, interval);
  projectPoint(src, pt2, x, interval + 1);
  if(interval[0] > interval[1])
  {
    double tmp = interval[0];
...
__device__
void intervalsIntersect(double interval1[2], double interval2[2], bool *ret)
{
  *ret = interval1[0] < interval2[1] && interval2[0] < interval1[1];
}
  ...
  for(int v=index; v<scanner.num_views; v+=stride)
  {
    ...
    for(int d=0; d<scanner.num_detectors; d++)
    {
      ...
      for(int c=0; c<scanner.num_pixels; c++)
      {
        ...
        double cos_correction1 = src[0]/sqrt((det_proj_interval[0] - src[1])*(det_proj_interval[0] - src[1]) + src[0]*src[0]);
        double cos_correction2 = src[0]/sqrt((det_proj_interval[1] - src[1])*(det_proj_interval[1] - src[1]) + src[0]*src[0]);
        double cos_correction = abs(0.5*(cos_correction1 + cos_correction2));
        
        double pxb1;
        projectPoint(src, det1, px_x, &pxb1);
        double pxb2;
        projectPoint(src, det2, px_x, &pxb2);
        if(pxb1 > pxb2)
        {
          double tmp = pxb1;
          pxb1 = pxb2;
        ...
        int px_bound1 = max((int)floor((pxb1 + col_begin)/px_width), 0);
        int px_bound2 = min((int)ceil((pxb2 + col_begin)/px_width), scanner.num_pixels);
        ...
        for(int r=px_bound1; r<px_bound2; r++)
        {
          ...
          bool ii;
          intervalsIntersect(px_proj_interval, det_proj_interval, &ii);
          
          if(ii)
          {
            double det_width = det_proj_interval[1] - det_proj_interval[0];
            double det_px_overlap = min(det_proj_interval[1], px_proj_interval[1]) - max(det_proj_interval[0], px_proj_interval[0]); 
